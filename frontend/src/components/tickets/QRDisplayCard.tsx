...
export default function QRDisplayCard({ ticket, event, onDownload, onPrint }: QRDisplayCardProps) {
  ...
  return (
    <div ref={tiltRef} className="tilt-card mx-auto max-w-sm overflow-hidden rounded-card border border-brand-100 bg-white shadow-lg">
      ...
      <div className="p-5 text-center">
        ...
        <div className="mt-4 grid gap-2 text-left text-xs font-semibold text-slate-600">
          <div className="flex items-center gap-2 rounded-control bg-slate-50 px-3 py-2">
        ...
        {hasQrPayload && (
          <div className="mt-5 grid grid-cols-2 gap-2">
            <button
              onClick={onDownload}
              className="btn-press inline-flex h-10 items-center justify-center gap-2 rounded-control border border-slate-200 text-sm font-medium text-slate-700 hover:bg-slate-50"
            >
              <Download className="h-4 w-4" />
              Tải vé
            </button>
            <button onClick={onPrint} className="btn-press inline-flex h-10 items-center justify-center gap-2 rounded-control bg-brand-600 text-sm font-medium text-white hover:bg-brand-700">
