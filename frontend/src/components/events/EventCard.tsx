  ...
  return (
    <article className="enterprise-card hover-lift group flex h-full flex-col overflow-hidden text-left">
      <div className="relative h-44 bg-slate-100">
        <EventBanner src={event.bannerUrl} alt={event.title} category={event.category} className="h-44 w-full transition duration-300 group-hover:scale-[1.03]" />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/55 via-slate-950/5 to-transparent" />
        <div className="absolute left-3 top-3 flex flex-wrap gap-2">
          <span className="rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-extrabold text-brand-800 shadow-sm backdrop-blur">
            {event.category}
          </span>
          <StatusBadge type="event" status={event.status} />
        </div>
        <div className="absolute bottom-3 left-3 right-3">
          <p className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-white/80">{event.clubName}</p>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
          <div>
            ...
            <p className={`mt-1 text-sm font-black ${isSoldOut ? "text-orange-600" : "text-emerald-600"}`}>
          ...
          <div className="flex gap-2">
            {onViewDetails && (
              <button
                ...
                className="btn-press min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700 hover:bg-slate-50"
              >
                {actionText}
              </button>
            )}
            {onRegister && event.status === "OPEN" && !isSoldOut && (
              <button
                ...
                className="btn-press inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-brand-600 px-3 text-sm font-bold text-white shadow-sm hover:bg-brand-700"
              >
